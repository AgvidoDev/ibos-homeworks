# Домашнее задание к занятию «ОС Windows (часть 1)» Test
...
```
System / PID 4 / Parent PID 0
```
2. Какой "драйвер" загружается первым? Что это за драйвер (ответьте своими словами)?
```
PROCMON24.SYS
Драйвер procmon24.sys отвечает за мониторинг и запись активности процессов при загрузке операционной системы Windows. 
Этот драйвер записывает в лог информацию о всех процессах, выполняемых при загрузке Windows и входе пользователя.
...
![](pic/taskmgr.png)
```
Windows Logon Application (winlogon.exe) — это важный компонент операционной системы Windows. 
Он отвечает за управление процедурами входа и выхода пользователя в системе.

Другими словами, если его завершить, windows равершит работу пользователя и заново зайдет в интерфейс
